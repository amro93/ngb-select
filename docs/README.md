...
``` ### 5. Searchable Multi-Select with Checkboxes & Chips Enable multiple selection with live
search filtering, "Select All" header, custom search placeholder, and removable chip badges: ```html
<ngb-select
  [options]="countries"
...
``` ### 6. RTL (Right-to-Left) & Arabic Support Full compatibility with Bootstrap 5 RTL and Arabic
search text normalization: ```html
<div dir="rtl" lang="ar">
  <ngb-select
...
``` ## 📖 Deep-Dive Documentation For full API property lists, architecture decisions, and
implementation strategies, refer to the [docs/](./docs) directory: - [Architecture &
